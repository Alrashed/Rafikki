...
static CGFloat const kJPSThumbnailAnnotationViewStandardHeight    = 65.0f;//65
static CGFloat const kJPSThumbnailAnnotationViewExpandOffset      = 200.0f;
static CGFloat const kJPSThumbnailAnnotationViewVerticalOffset    = 34.0f;//34.0f
...
- (void)setupImageView {
    ...
    UIImageView *pinImageView=[[UIImageView alloc] initWithFrame:self.bounds];
    pinImageView.image=[UIImage imageNamed:@"pin.png"];
    pinImageView.contentMode=UIViewContentModeScaleAspectFit;
    [self addSubview:pinImageView];
    
    _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(8.0, 4.0, 38, 38)];
...
- (void)setupTitleLabel {
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(-50.0f, 10.0f, 175.0f, 15.0f)];
...
- (void)setupSubtitleLabel {
    _subtitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(-50.0f, 25.0f, 175.0f, 15.0f)];
...
- (void)setupDisclosureButton {
    ...
    _disclosureButton.frame = CGRectMake(kJPSThumbnailAnnotationViewExpandOffset/2.0f + self.frame.size.width/2.0f,
                                         15.5f,
...
- (void)setLayerProperties {
    ...
    _bgLayer.path = path;
    CFRelease(path);
    _bgLayer.fillColor = [UIColor colorWithRed:12/255.0 green:162/255.0 blue:218/255.0 alpha:1].CGColor;
    
    
    _bgLayer.shadowColor = [UIColor whiteColor].CGColor;
...
- (void)updateWithThumbnail:(JPSThumbnail *)thumbnail {
    ...
    if ([thumbnail.image length]>7) {
        
        [self.imageView setImageWithURL:[NSURL URLWithString:thumbnail.image] placeholderImage:[UIImage imageNamed:@"photo"] usingActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
    }
    else
    {
        self.imageView.image=[UIImage imageNamed:@"userprofile.png"];
    }
...
- (CGPathRef)newBubbleWithRect:(CGRect)rect {
    CGFloat stroke = 5.0f;//1
	CGFloat radius = 7.0f;//7
